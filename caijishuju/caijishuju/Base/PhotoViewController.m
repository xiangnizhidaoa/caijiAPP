...
-(void)initView{
    ...
    self.scrollerView.pagingEnabled = YES;
        ...
        if (self.type == 0) {
            NSInteger number;
            if ([self.invImageArray.lastObject isEqualToString:@"上传图片"]) {
                number = self.invImageArray.count - 1;
            }else{
                number = self.invImageArray.count;
            }
            self.scrollerView.contentSize = CGSizeMake(SCREEN_WIDTH * number, SCREENH_HEIGHT );
            self.title =  [NSString stringWithFormat:@"%ld/%lu",self.locationView + 1,(unsigned long)number];
            for (int i = 0; i < number; i ++) {
                YYAnimatedImageView *imageView = [[YYAnimatedImageView alloc] initWithFrame:CGRectMake(SCREEN_WIDTH * i, 0, SCREEN_WIDTH, SCREENH_HEIGHT )];
                [imageView sd_setImageWithURL:[NSURL URLWithString:[self.invImageArray[i] stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet characterSetWithCharactersInString:self.invImageArray[i]]]] placeholderImage:[UIImage imageNamed:@"pic.png"]];
                imageView.userInteractionEnabled = YES;
                imageView.contentMode = UIViewContentModeScaleAspectFit;//设置图片填充样式
                //图片点击手势
...
//                [imageView addGestureRecognizer:longPress];
            ...
            self.title =  [NSString stringWithFormat:@"%ld/%lu",self.locationView + 1,(unsigned long)self.imageArray.count];
            for (int i = 0; i < self.imageArray.count; i ++) {
                YYAnimatedImageView *imageView = [[YYAnimatedImageView alloc] initWithFrame:CGRectMake(SCREEN_WIDTH * i, 0, SCREEN_WIDTH, SCREENH_HEIGHT )];
                
//                [imageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@?fileId=%@",BS_Url.getFileUrl,[self.imageArray objectAtIndex:i]]]placeholderImage:[UIImage imageNamed:[self.imageArray objectAtIndex:i]]];
                imageView.userInteractionEnabled = YES;
                imageView.contentMode = UIViewContentModeScaleAspectFit;//设置图片填充样式
                //图片点击手势
                UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(cilck)];
...
-(void)deleat//点击删除弹出提示框(是否确定删除)
{
    ...
    UIAlertAction* defaultAction = [UIAlertAction actionWithTitle:@"是" style:UIAlertActionStyleDefault
                                                          handler:^(UIAlertAction * action) {
                                                              ...
                                                              if (self.type == 0) {
                                                                  [[NSNotificationCenter defaultCenter] postNotificationName:@"deleat" object:self userInfo:@{@"value":self.invImageArray[self.index],@"index":@(self.index)}];
                                                              }else{
                                                                  [[NSNotificationCenter defaultCenter] postNotificationName:@"deleat" object:self userInfo:@{@"value":self.imageArray[self.index],@"index":@(self.index)}];
                                                              }
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if (self.type == 0) {
        self.title = [NSString stringWithFormat:@"%ld/%lu",self.index+ 1,(unsigned long)number];
    }else{
